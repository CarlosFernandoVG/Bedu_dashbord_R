...
md %>% filter(Home.team == local) %>%
  ...
  labs(x = NULL, title = paste(c("Número de goles del equipo ", local, " como local y del equipo visitante"), collapse = "")) + 
...
```{r}
md %>% filter(Home.team == local) %>% arrange(Away.team)
```
